...
const KEY = "ff_commit_v1"

// Rotate these (update names if yours differ)
...
export default function EntryGate({ alwaysShow = false, onEnter }) {
  ...
  // Decide if we should show the gate
  useEffect(() => {
    if (alwaysShow) { setShow(true); return }
    try {
      const raw = localStorage.getItem(KEY)
      if (!raw) { setShow(true); return }
      const { t } = JSON.parse(raw)
      setShow(Date.now() - Number(t) > REMEMBER_MS)
    } catch { setShow(true) }
  }, [alwaysShow])
  ...
  function handleEnter(){
    try { localStorage.setItem(KEY, JSON.stringify({ t: Date.now() })) } catch {}
    setShow(false); onEnter?.()
  }
  ...
  return (
    <div style={bgWrap}>
      ...
      <div style={Card} className="vstack">
        ...
        <button onClick={handleEnter} style={LinkBtn}>Skip for now</button>

        <div style={{ marginTop: 6, ...Small }}>We’ll remember this for 24 hours on this device.</div>
      </div>
    </div>
  )
}
