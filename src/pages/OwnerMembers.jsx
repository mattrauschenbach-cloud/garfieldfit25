...
export default function OwnerMembers(){
  ...
  async function updateRole(uid, role){
    ...
    try { setBusy(uid); await setDoc(doc(db, "profiles", uid), { role }, { merge: true }) }
        ...
        <table style={{width:"100%", borderCollapse:"collapse"}}>
          <thead>
            <tr style={{background:"#0f1a30"}}>
              <th style={th}>Name</th><th style={th}>Email</th><th style={th}>UID</th><th style={th}>Role</th><th style={th}></th>
            </tr>
          </thead>
          <tbody>
            {rows.map(r => (
              <tr key={r.id} style={{borderTop:"1px solid #1f2937"}}>
                ...
                <td style={td}>
                  <select value={r.role || "member"} onChange={e => updateRole(r.id, e.target.value)} disabled={busy === r.id} style={select}>
...
const select = { background:"#0b1426", color:"#e5e7eb", border:"1px solid #1f2937", borderRadius:8, padding:"8px 10px" }
